...
Finally, note that while Groovy extra properties can be modified using an object's `ext` property, in Kotlin they are modified using the `extra` property.
It is important to look at each object and update the build scripts accordingly.
For example, `ext {springVersion = "3.1.0.RELEASE"}` becomes `val springVersion by extra("3.1.0.RELEASE")`.

Examples with additional details for handling extra properties can be found in <<kotlin_dsl#kotdsl:extra_properties,the Kotlin DSL primer>>.
